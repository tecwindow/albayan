...
class UpdateChecker(QThread):
    ...
    def run(self):
        try:
            ...
            logger.error(
                f"Error during the update check: {ErrorMessage(e)}", exc_info=True
            )
...
class UpdateManager:
    def __init__(self, parent, auto_update=False):
        ...
        logger.info(
            f"UpdateManager initialized with auto_update set to {self.auto_update}"
        )
    ...
    def check_auto_update(self) -> None:
        try:
            ...
            if self.auto_update:
                ...
                logger.debug(
                    "Making a background request to open session for update check."
                )
                Thread(
                    target=UpdateChecker.session.get,
                    kwargs={"url": UpdateChecker.url},
                    daemon=True,
                ).start()
        except Exception as e:
            logger.error(
                f"Error in check_auto_update: {ErrorMessage(e)}", exc_info=True
            )
    ...
    def check_updates(self):
        ...
        self.update_checker.update_available.connect(
            lambda info: self.on_update_available(info)
        )
        self.update_checker.update_error.connect(
            lambda error: self.on_update_error(error)
        )
    ...
    def on_update_available(self, info: dict):
        ...
        try:
            ...
            if version.parse(latest_version) > version.parse(program_version):
                ...
                release_notes = info.get("release_notes", {}).get(
                    language, info["release_notes"].get("Arabic", "")
                )
                download_url = info.get("download_url")
                logger.info(
                    f"A new version {latest_version} is available. Displaying update dialog..."
                )
            ...
            logger.error(
                f"Error in on_update_available: {ErrorMessage(e)}", exc_info=True
            )
    ...
    def show_update_dialog(self, release_notes, download_url, latest_version):
        try:
            ...
            UpdateDialog(
                self.parent, release_notes, download_url, latest_version
            ).exec()
        except Exception as e:
            logger.error(
                f"Error in show_update_dialog: {ErrorMessage(e)}", exc_info=True
            )

    def show_no_update_dialog(self):
        if not self.auto_update:
            logger.debug(
                f"No update found, displaying 'no update' dialog for version {program_version}."
            )
            msg_box = QMessageBox(self.parent)
