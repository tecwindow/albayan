...
class FilterManager(QObject):
    filterModeChanged = Signal(bool)
    activeCategoryChanged = Signal(str)
    itemSelectionChanged = Signal(QComboBox, int)
    itemeSelected = Signal()
    selectOutOfRange = Signal(int)
    filteredItemsUpdated = Signal(QComboBox, list, str)
    searchQueryUpdated = Signal(str)
    ...
    def set_category(
        self, id, label: str, items: List[Item], widget: QComboBox
    ) -> None:
    ...
    def change_category_items(self, id: int, new_items: List[Item]) -> None:
        ...
        logger.debug(
            f"Changing items for category ID={id}. New item count: {len(new_items)}."
        )
    ...
    def switch_category(self, direction: int) -> None:
        ...
        logger.debug(
            f"Switching category. Current index: {self.current_category_index}, Direction: {direction}."
        )
        if not self.categories:
            logger.warning("No categories available to switch.")
            return
        self.current_category_index = (self.current_category_index + direction) % len(
            self.categories
        )
    ...
    def navigate_items(self, direction: int) -> None:
        ...
        logger.debug(
            f"Navigating items. Current category index: {self.current_category_index}, Direction: {direction}."
        )
        if not self.categories:
        ...
        logger.debug(
            f"Navigated to item index {new_index} in category {active_category.label}."
        )
    ...
    def delete_last_char(self):
        ...
        if active_category.search_query:
            ...
            self.searchQueryUpdated.emit(active_category.search_query)
            logger.debug(
                f"Updated search query after deletion: {active_category.search_query}."
            )
            self.update_filtered_items()
    ...
    def update_filtered_items(self):
        ...
        filtered_items = [
            ...
            if item.text.startswith(active_category.search_query)
        ]
        self.filteredItemsUpdated.emit(
            combo_box, filtered_items, combo_box.currentText()
        )
        logger.debug(
            f"Filtered items updated for category {active_category.label}. "
    ...
    def clear_filters(self):
        ...
        for category in self.categories:
            ...
            self.filteredItemsUpdated.emit(
                combo_box, category.items, combo_box.currentText()
            )
    ...
    def handle_key_press(self, event: QKeyEvent) -> bool:
        ...
        if event.modifiers() == Qt.ControlModifier and event.key() == Qt.Key_F:
            ...
            if event.key() == Qt.Key.Key_Left:
                ...
                logger.debug(
                    "Down arrow key detected, navigating down in the item list."
                )
                self.navigate_items(1)
                return True
            elif event.key() == Qt.Key.Key_Home:
                logger.debug(
                    "Home key detected, jumping to the first item in the list."
                )
                ...
                logger.debug(
                    "Backspace key detected, deleting last character from search query."
                )
                self.delete_last_char()
                return True
            elif event.key() in (Qt.Key.Key_Return, Qt.Key.Key_Enter):
                logger.debug(
                    "Enter/Return key detected, selecting item and exiting filter mode."
                )
                ...
                logger.debug(
                    f"Arabic character detected: {event.text()}, filtering items."
                )
