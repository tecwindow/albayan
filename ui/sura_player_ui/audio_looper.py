...
class AudioLooper:
    ...
    def __init__(self, parent, player: SurahPlayer, loop_check_interval: int = 100):
        ...
        logger.debug(
            f"AudioLooper initialized with check interval: {loop_check_interval} ms"
        )
    ...
    def set_loop_start(self):
        ...
        UniversalSpeech.say(
            f"تم تحديد البداية عند: {self.parent.format_time(self.loop_start)}.",
            force=True,
        )
    ...
    def set_loop_end(self):
        ...
        UniversalSpeech.say(
            f"تم تحديد النهاية عند: {self.parent.format_time(self.loop_end)}.",
            force=True,
        )
    ...
    def toggle_loop(self):
        ...
        if self.loop_active:
            logger.debug(
                f"Loop started from {self.loop_start} ms to {self.loop_end} ms."
            )
            UniversalSpeech.say(
                f"بدأ التكرار من {self.parent.format_time(self.loop_start)} إلى {self.parent.format_time(self.loop_end)}.",
                force=True,
            )
            ...
            self.player.play()
            logger.debug(
                f"Loop started from {self.parent.format_time(self.loop_start)} to {self.parent.format_time(self.loop_end)}."
            )
    ...
    def check_loop(self):
        ...
        if current_position >= self.loop_end or current_position < self.loop_start:
            logger.debug(
                f"Loop end reached at {self.parent.format_time(current_position)}. Restarting loop after {self.loop_delay} ms delay."
            )
    ...
    def restart_loop(self):
        ...
        if self.loop_active:
            self.player.set_position(self.loop_start)
            logger.debug(
                f"Restarting loop from {self.parent.format_time(self.loop_start)} to {self.parent.format_time(self.loop_end)}."
            )
            self.player.play()
            self.monitor_timer.start()
    ...
    def return_to_start(self):
        ...
        self.player.set_position(self.loop_start)
        logger.debug(
            f"Returning to loop start at {self.parent.format_time(self.loop_start)}."
        )
        UniversalSpeech.say(
            f"يتم التشغيل من: {self.parent.format_time(self.loop_start)}.", force=True
        )
