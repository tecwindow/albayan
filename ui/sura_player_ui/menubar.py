...
class MenuBar(QMenuBar):
    def __init__(self, parent=None):
        ...
        loop_menu.addActions(
            [
                self.set_start_action,
                self.set_end_action,
                self.return_to_start_action,
                self.clear_loop_action,
    ...
    def get_player_actions(self) -> List[QAction]:
        return [
            ...
            self.rewind_action,
            self.replay_action,
            self.up_volume_action,
            self.down_volume_action,
            self.next_surah_action,
            self.previous_surah_action,
            self.next_reciter_action,
            self.previous_reciter_action,
            self.set_start_action,
            self.set_end_action,
            self.return_to_start_action,
            self.clear_loop_action,
    ...
    def eventFilter(self, obj, event: QEvent):
        if obj == self:
            if (
                ...
                and event.key() in (Qt.Key.Key_Escape, Qt.Key.Key_Enter)
            ) or (event.type() in (QEvent.Type.Close, QEvent.Type.Hide)):
                try:
                    self.clearFocus()
