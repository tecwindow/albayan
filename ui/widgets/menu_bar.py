import os
import sys
from PySide6.QtWidgets import QApplication, QMenuBar, QMenu, QMessageBox
...
class MenuBar(QMenuBar):
    ...
    def create_menu(self):
        ...
        self.go_to_saved_position_action.triggered.connect(
            lambda: self.parent.set_text(True)
        )
        self.go_to_saved_position_action.triggered.connect(
            lambda: Globals.effects_manager.play("move")
        )
        ...
        self.navigation_menu.addActions(
            [
                self.next_action,
                self.previous_action,
                self.search_action,
                self.go_to_saved_position_action,
                self.go_to_ayah_action,
                self.go_to_action,
                self.quick_access_action,
                self.close_action,
                self.exit_action,
            ]
        )
        ...
        self.rewind_action.triggered.connect(
            lambda: self.parent.toolbar.player.rewind(Config.listening.forward_time)
        )
        self.forward_action = QAction("تقديم", self)
        self.forward_action.triggered.connect(
            lambda: self.parent.toolbar.player.forward(Config.listening.forward_time)
        )
        self.replay_action = QAction("إعادة", self)
        self.replay_action.triggered.connect(
            lambda: self.parent.toolbar.player.set_position(0)
        )
        ...
        self.actions_menu.addActions(
            [
                self.save_position_action,
                self.save_bookmark_action,
                self.verse_tafsir_action,
                self.verse_info_action,
                self.verse_grammar_action,
                self.copy_verse_action,
            ]
        )
        ...
        for name, mode, shortcut in modes:
            ...
            action.triggered.connect(
                lambda checked, m=mode: self.parent.OnChangeNavigationMode(m.value)
            )
        ...
        self.info_menu.addActions(
            [
                self.ayah_info_action,
                self.surah_info_action,
                self.page_info_action,
                self.quarter_info_action,
                self.hizb_info_action,
                self.juz_info_action,
                self.moshaf_info_action,
            ]
        )
        ...
        self.tools_menu.addActions(
            [
                self.sura_player_action,
                self.athkar_action,
                self.bookmark_manager_action,
                self.tasbih_action,
                self.message_for_you_action,
                self.download_manager_action,
            ]
        )
        ...
        self.change_after_listening_action.triggered.connect(
            self.open_after_listening_settings
        )

        self.change_repeat_limit_action = QAction("تغيير عدد مرات تكرار الآية", self)
        self.change_repeat_limit_action.triggered.connect(
            self.open_repeat_limit_settings
        )
        ...
        self.whats_new_action.triggered.connect(
            lambda: self.open_documentation("Whats_new")
        )
        self.shortcuts_action = QAction("اختصارات لوحة المفاتيح", self)
        self.shortcuts_action.triggered.connect(
            lambda: self.open_documentation("Shortcuts")
        )
        ...
        self.help_menu.addActions(
            [
                ...
                self.whats_new_action,
                self.shortcuts_action,
                self.marks_action,
                self.update_program_action,
                self.open_log_action,
                self.about_program_action,
            ]
        )
    ...
    def OnSettings(self):
        ...
        self.close_action.setVisible(Config.general.run_in_background_enabled)
        logger.debug(
            "Settings dialog closed, setting visibility of close action as {Config.general.run_in_background_enabled}."
        )
    ...
    def OnContact(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "خطأ", "حدث خطأ أثناء محاولة فتح برنامج البريد الإلكتروني"
            )
    ...
    def OnGoTo(self):
        ...
        logger.debug(
            f"Current position: {current_position}, Max: {max_position}, navigation_mode: {self.parent.quran_manager.navigation_mode}"
        )
        go_to_dialog = GoToDialog(
            self.parent,
            title=title,
            initial_value=current_position,
    ...
    def OnGoToAyah(self):
        ...
        combo_data[current_ayah.sura_number][
            "initial_value"
        ] = current_ayah.number_in_surah
        category_label = self.parent.quran_manager.view_content.label
        ...
        go_to_dialog = GoToDialog(
            self.parent,
            title=title,
            initial_value=current_ayah.sura_name,
            combo_data=combo_data,
            style=GoToStyle.NUMERIC_FIELD
            | GoToStyle.COMBO_FIELD
            | GoToStyle.CHECKBOXFIELD,
        )
        ...
        go_to_dialog.checkbox_field.setChecked(
        ...
        if go_to_dialog.exec():
            ...
            Config.listening.auto_play_ayah_after_go_to = (
                go_to_dialog.checkbox_field.isChecked()
            )
            Config.save_settings()
            ayah = self.parent.quran_manager.view_content.get_by_ayah_number_in_surah(
                ayah_number_in_surah, surah_number
            )
    ...
    def quit_application(self) -> bool:
        ...
        if self.check_active_downloads():
            if not Config.downloading.show_incomplete_download_warning:
                logger.debug(
                    "Active downloads found, stopping all without confirmation due to settings."
                )
    ...
    def check_active_downloads(self) -> bool:
        ...
        return (
            self.surah_manager.has_active_downloads()
            or self.ayah_manager.has_active_downloads()
        )
    ...
    def confirm_stop_downloads(self) -> bool:
        ...
        msg_box.setText(
            "توجد عمليات تنزيل لم تكتمل بعد. سيؤدي الخروج إلى إيقافها. هل تريد المتابعة؟"
        )
        yes_button = msg_box.addButton(
            "إيقاف التنزيل والخروج", QMessageBox.ButtonRole.AcceptRole
        )
