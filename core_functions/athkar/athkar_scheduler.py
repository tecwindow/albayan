...
class AthkarScheduler:
    def __init__(
        self,
        athkar_db_folder: Union[Path, str],
        default_category_path: Optional[Union[Path, str]] = None,
        text_athkar_path: Optional[Union[Path, str]] = None,
        default_category_settings: Optional[Dict[str, int]] = None,
    ) -> None:
        logger.debug(
            f"Initializing AthkarScheduler with db folder: {athkar_db_folder}, default category path: {default_category_path}, text athkar path: {text_athkar_path}"
        )
        self.default_category_path = (
            Path(default_category_path)
            if isinstance(default_category_path, str)
            else default_category_path
        )
        self.text_athkar_path = (
            Path(text_athkar_path)
            if isinstance(text_athkar_path, str)
            else text_athkar_path
        )
        self.default_category_settings = (
            default_category_settings if default_category_settings is not None else {}
        )
    ...
    def setup(self) -> None:
        ...
        if self.default_category_path:
            logger.debug(
                f"Ensuring category folder exists: {self.default_category_path}"
            )
            self.default_category_path.mkdir(parents=True, exist_ok=True)
            try:
                category_id = self.db_manager.create_category(
                    "default",
                    str(self.default_category_path),
                    **self.default_category_settings,
                )
                ...
                if self.text_athkar_path and self.text_athkar_path.exists():
                    with open(self.text_athkar_path, encoding="UTF-8") as f:
                        ...
                        logger.debug(
                            f"Loaded {len(text_data)} text athkar into default category"
                        )
        ...
        for category in self.categories:
            refresher = AthkarRefresher(
                self.db_manager, category.audio_path, category.id
            )
            refresher.refresh_data()

    def audio_athkar_job(self, category_id: int, audio_path: str) -> None:
        logger.debug(
            f"Starting audio athkar for category ID {category_id} using {audio_path}"
        )
        with AthkarPlayer(
            audio_path, self.db_manager.get_audio_athkar(category_id)
        ) as player:
            player.play()
        ...
        for category in self.categories:
            try:
                ...
                triggers = self._create_triggers(
                    from_time, to_time, category.play_interval
                )
                for trigger in triggers:
                    self._add_jobs(category, trigger)
                    logger.debug(
                        f"Scheduled athkar jobs for category {category.id} from {category.from_time} to {category.to_time}"
                    )
            except Exception as e:
                logger.error(
                    f"Error scheduling category {category.id}: {e}", exc_info=True
                )
